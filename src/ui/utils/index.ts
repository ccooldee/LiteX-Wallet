...
export function balanceToSixDecimalDigit(val: string) {
  if (val === '0') return val;
  let balance = parseFloat(val).toFixed(6).toString();
  while(balance[balance.length - 1] === '0'){
    balance = balance.slice(0,balance.length-1);
  }
  if (balance[balance.length - 1] === '.') return balance.slice(0,balance.length-1);
  return balance;
